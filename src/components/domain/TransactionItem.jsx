    ...
    >
      ...
      <div className="flex-1 min-w-0 pr-4">
        <div className="flex items-center gap-2">
            <h4 className="font-semibold text-blue-50 truncate text-base">{item.description}</h4>
            {/* INTEGRITY BADGE */}
            {isVerified ? (
                <ShieldCheck className="w-3 h-3 text-emerald-400" title="Verified by Bank Statement" />
            ) : (
                <User className="w-3 h-3 text-slate-500" title="Manual User Entry" />
            )}
        </div>
      ...
      <div className="text-right shrink-0 flex flex-col items-end">
        ...
        <div className="flex gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {onEdit && (
                <button onClick={(e) => { e.stopPropagation(); onEdit(item); }} className="p-2 text-slate-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-full">
                    <Edit2 className="w-3.5 h-3.5" />
                </button>
            )}
            {onDelete && (
                <button onClick={(e) => { e.stopPropagation(); onDelete(item.id); }} className="p-2 text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-full">
