...
import { 
  ...
  ShieldAlert, Bot, Sparkles, X, CheckCircle2, Briefcase, 
  Landmark, Activity, GraduationCap, Loader2, Clock, 
  UserCog, Wand2, ArrowDownCircle, Scale, FileCheck, ArrowRight
} from 'lucide-react';
...
const DeductionBar = ({ label, used, limit, color = "bg-blue-500", pace = 0 }) => (
...
const TaxActionCard = ({ action, section, savings, deadline }) => (
    <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-900/40 to-emerald-800/20 border border-emerald-500/30 relative overflow-hidden group hover:border-emerald-500/50 transition-all print:border-gray-300 print:bg-white print:text-black break-inside-avoid">
        ...
        <h4 className="font-bold text-white text-sm mb-1 print:text-black">{action}</h4>
        <p className="text-xs text-emerald-200/60 print:text-slate-600">
            Potential Savings: <span className="text-emerald-300 font-bold print:text-black">{savings}</span>
        </p>
    </div>
);
            ...
            <div className="bg-[#0f0c29] w-full max-w-sm p-6 rounded-[2rem] border border-white/10 shadow-2xl relative">
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div 
                        ...
                        onClick={() => setLocalProfile({...localProfile, isBusiness: !localProfile.isBusiness})}
                    >
                        <div className={`w-5 h-5 rounded-md border flex items-center justify-center ${localProfile.isBusiness ? 'bg-blue-500 border-blue-500' : 'border-slate-500'}`}>
                            {localProfile.isBusiness && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                        </div>
                        <div className="flex flex-col">
                            <span className="text-sm font-medium text-white">I have Business Income</span>
                            <span className="text-[10px] text-slate-400">Select if you earn apart from Salary</span>
                        </div>
                    </div>
                    ...
                    {fields.map(field => (
                        <div key={field.key} className="space-y-2">
                            <div className="flex justify-between items-center">
                                ...
                                {field.detected > 0 && (
                                    <button 
                                        onClick={() => setLocalProfile({...localProfile, [field.key]: field.detected})} 
                                    ...
                                    >
                                        <ArrowDownCircle className="w-3 h-3" /> Use Detected ₹{formatIndianCompact(field.detected)}
                                    </button>
                                )}
                            </div>
                            <input 
                                ...
                                placeholder={field.detected > 0 ? `Detected: ${field.detected}` : "0"} 
                            />
                        </div>
                    ))}
                </div>
...
// --- MAIN PAGE COMPONENT ---
const AuditPage = ({ transactions, wealthItems, taxProfile, onUpdateProfile, showToast, settings, setActiveTab }) => {
    const [adviceCards, setAdviceCards] = useState([]);
    const [loading, setLoading] = useState(false);
    const [showWizard, setShowWizard] = useState(false);

    // 1. CALCULATE TAX DATA
    const data = useMemo(() => {
    ...
    const detectedValues = useMemo(() => {
        ...
        transactions.forEach(t => {
            ...
            if (t.type === 'expense') {
                ...
                if (desc.includes('ppf') || desc.includes('lic') || desc.includes('elss')) result.epf += val;
                if ((desc.includes('health') || desc.includes('mediclaim')) && !desc.includes('lic')) result.health += val;
    ...
    const integrityScore = useMemo(() => {
        ...
        const verifiedCount = transactions.filter(t => t.verificationStatus === 'verified' || t.confidence > 0).length;
        return Math.round((verifiedCount / transactions.length) * 100);
    }, [transactions]);

    let integrityColor = "text-emerald-400";
    let integrityLabel = "Audit Ready";
    if (integrityScore < 50) { integrityColor = "text-rose-400"; integrityLabel = "High Audit Risk"; }
    else if (integrityScore < 80) { integrityColor = "text-amber-400"; integrityLabel = "Verification Needed"; }
    ...
    const getAdvice = async () => {
        ...
        try {
            const contextData = JSON.stringify({
                ...
                income: { total: data.sources.total, salary: data.heads.salary, business: data.heads.business },
    ...
    return (
        <div className="space-y-6 pb-24 animate-in fade-in print:pb-0 print:text-black print:bg-white">
            
            {/* Header */}
            <div className="flex justify-between items-start print:hidden">
                <div>
                    <h2 className="text-2xl font-bold text-white mb-2">Tax Scout</h2>
                    <div className="flex flex-wrap gap-2">
                        <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-white/5 text-slate-400 border border-white/10 flex items-center gap-1">
                            <Wand2 className="w-3 h-3" /> Analyzed {transactions.length} Txns
                        </span>
                        {userPersona.map(tag => (
                ...
                <div className="flex gap-2">
                    <button onClick={() => window.print()} className="p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors text-slate-300">
                        <Printer className="w-5 h-5" />
                    </button>
                    <button onClick={() => setShowWizard(true)} className="p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors text-white">
            ...
            <div className="hidden print:block text-center mb-8 border-b-2 border-black pb-4">
                <h1 className="text-3xl font-bold text-black uppercase tracking-wider">Tax Audit Report</h1>
                <div className="flex justify-between mt-4 text-sm text-gray-600">
                    <p><strong>Name:</strong> {settings?.name || "User"}</p>
                    <p><strong>FY:</strong> {data.fiscalYear}</p>
                    <p><strong>Date:</strong> {new Date().toLocaleDateString()}</p>
                </div>
            </div>

            {/* ACTION CARD: Go To Filing */}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-blue-200">
                                Your estimated tax is <span className="font-bold text-white">₹{Math.min(data.taxNew, data.taxOld).toLocaleString()}</span>. 
            ...
            <div className="bg-gradient-to-br from-indigo-900/40 to-blue-900/40 backdrop-blur-xl p-6 rounded-[2rem] border border-white/10 shadow-2xl relative overflow-hidden print:border print:border-gray-300 print:shadow-none print:bg-none print:text-black break-inside-avoid">
                <div className="relative z-10 grid grid-cols-2 gap-6">
                    <div>
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 print:text-gray-600">Taxable Income</p>
                        <h3 className="text-2xl font-bold text-white print:text-black">₹{data.taxableOld.toLocaleString()}</h3>
                        <p className="text-[10px] text-indigo-300 mt-1 print:text-gray-500">FY {data.fiscalYear}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 print:text-gray-600">Recommended</p>
                        <div className={`inline-block px-3 py-1 rounded-lg text-sm font-bold ${data.taxNew < data.taxOld ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'} print:bg-gray-100 print:text-black print:border print:border-gray-300`}>
                            {data.taxNew < data.taxOld ? 'NEW REGIME' : 'OLD REGIME'}
                        </div>
                        <p className="text-[10px] text-slate-400 mt-1 print:text-gray-500">
                            Est. Liability: ₹{Math.min(data.taxNew, data.taxOld).toLocaleString()}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="flex justify-between items-start mb-6 relative z-10">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h3 className="font-bold text-white text-lg tracking-tight">AI Consultant</h3>
                        </div>
                        <p className="text-xs text-slate-400">Tailored for {userPersona[0]}</p>
                    </div>
                    <button onClick={getAdvice} disabled={loading} className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-lg shadow-emerald-900/20 flex items-center gap-2">
                ...
                <div className="relative z-10 min-h-[50px]">
                    {!loading && adviceCards.length === 0 && (
                        <div className="text-center py-6 text-slate-500 text-xs border border-dashed border-slate-700 rounded-xl">
                            Tap 'Identify Savings' to generate a plan specific to your profession.
                        </div>
                    )}
                    {adviceCards.length > 0 && (
                        <div className="grid gap-3 animate-in slide-in-from-bottom-4">
                            {adviceCards.map((card, idx) => <TaxActionCard key={idx} {...card} />)}
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 print:border-gray-300 print:bg-white print:text-black break-inside-avoid">
                ...
                <div className="space-y-3">
                    <div className="flex justify-between text-sm"><span className="text-slate-300 print:text-gray-700">1. Salary</span><span className="text-white font-medium print:text-black">₹{data.heads.salary.toLocaleString()}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-slate-300 print:text-gray-700">2. House Property</span><span className="text-white font-medium print:text-black">₹{data.heads.houseProperty.toLocaleString()}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-slate-300 print:text-gray-700">3. Business / Profession</span><span className="text-white font-medium print:text-black">₹{data.heads.business.toLocaleString()}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-slate-300 print:text-gray-700">4. Capital Gains</span><span className="text-yellow-400 font-medium print:text-black">₹{data.heads.capitalGains.toLocaleString()}*</span></div>
                    <div className="flex justify-between text-sm"><span className="text-slate-300 print:text-gray-700">5. Other Sources</span><span className="text-white font-medium print:text-black">₹{data.heads.other.toLocaleString()}</span></div>
                </div>
            </div>
            ...
            <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 print:border-gray-300 print:bg-white print:text-black break-inside-avoid">
                ...
                <DeductionBar label="80C Investments" used={data.deductions.c80.used} limit={data.deductions.c80.limit} color="bg-cyan-500" pace={data.mode === 'PLANNING' ? data.deductions.c80.pace : 0} />
                <DeductionBar label="80D Health Ins." used={data.deductions.d80.used} limit={data.deductions.d80.limit} color="bg-pink-500" />
                <DeductionBar label="NPS (80CCD 1B)" used={data.deductions.nps.used} limit={data.deductions.nps.limit} color="bg-purple-500" />
                ...
                {data.missedSavings > 0 && (
                    <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-3 print:border-gray-300 print:bg-gray-50">
                        ...
                        <div>
                            <p className="text-xs text-red-200 font-bold print:text-black">Leakage Detected</p>
                            <p className="text-[10px] text-red-300/70 print:text-gray-600">You are missing out on ~₹{Math.round(data.missedSavings).toLocaleString()} in potential tax savings.</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="p-4 rounded-[1.5rem] bg-white/5 border border-white/10 flex flex-col gap-3 print:border-gray-300 print:bg-white print:text-black break-inside-avoid">
                 <div className="flex justify-between items-start">
                    <div className="flex items-start gap-3">
                        <ShieldAlert className={`w-5 h-5 ${integrityScore < 80 ? 'text-amber-500' : 'text-emerald-500'} shrink-0 mt-0.5`} />
                        <div>
                            <h4 className="text-sm font-bold text-white mb-1 print:text-black">Data Integrity Score</h4>
                            <p className="text-xs text-slate-400 leading-relaxed print:text-gray-600">Based on verified vs. manual entries.</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <h3 className={`text-xl font-bold ${integrityColor} print:text-black`}>{integrityScore}%</h3>
                ...
                <div className="h-1.5 w-full bg-black/40 rounded-full overflow-hidden flex print:bg-gray-200">
                    <div className="h-full bg-emerald-500 print:bg-black" style={{ width: `${integrityScore}%` }}></div>
                </div>
                ...
                {integrityScore < 80 && (
                    <p className="text-[10px] text-rose-300 bg-rose-500/10 p-2 rounded-lg border border-rose-500/20 print:border-gray-300 print:bg-white print:text-black">
                        <strong>Note:</strong> Low verification score. Ensure transactions are backed by OCR or bank statements before filing.
                    </p>
                )}
            </div>
            ...
            <div className="text-center pt-8 pb-8 print:pb-4 border-t border-white/5 print:border-black mt-8">
                <p className="text-[10px] text-slate-600 print:text-gray-500">
                    <strong>DISCLAIMER:</strong> This report is a computer-generated estimate based on user-provided financial data.
                </p>
            </div>
