    ...
    try {
      ...
      if (response.ok) {
        if (isSignup) {
          ...
          onAuthSuccess({ id: data.user_id, username: data.username });
  ...
  return (
    <div className="min-h-screen w-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-900 via-slate-900 to-black text-white flex flex-col items-center justify-center p-6">
      <div className="z-10 w-full max-w-sm text-center">
        ...
        <p className="text-blue-200/70 mb-8 text-sm italic">Financial clarity for the modern era.</p>
        ...
        <div className="mt-8 text-[10px] text-slate-500 font-mono uppercase tracking-widest">{APP_VERSION}</div>
      </div>
    </div>
  );
};
